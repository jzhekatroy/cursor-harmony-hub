...
async function main() {
  ...
  await insertMany('client_events', (rows) => (prisma as any).clientEvent.createMany({ data: rows }), data.clientEvents)
  ...
  await insertMany('master_rotations', (rows) => (prisma as any).masterRotation.createMany({ data: rows }), data.masterRotations)
