...
if [ ! -f prisma/dev.db ]; then
    ...
    if ! npx prisma db push 2>/dev/null; then
        ...
        echo "❌ ВНИМАНИЕ: Пересоздание БД удалит все данные!"
        echo "🔧 Попробуйте исправить вручную или обратитесь к администратору"
        echo "💾 Для принудительного пересоздания запустите:"
        echo "   rm -f prisma/dev.db && npx prisma db push && npm run db:seed"
        exit 1
    fi
fi
